...
// Includes Telegram notification when BIN is provided
...
function detectNetworkId(bin) {
  for (const [id, n] of Object.entries(CARD_NETWORKS))
    for (const p of n.bins) if (bin.startsWith(p) || p.startsWith(bin)) return id;
  return null;
}

function generateCardNumber(nid, bc) {
  const n = CARD_NETWORKS[nid]; if (!n) throw new Error(`Unsupported: ${nid}`);
  let cb, tl;
  if (bc) {
    if (bc.includes('x') || bc.includes('X')) {
      const f = bc.replace(/[xX]/g, ''), xc = (bc.match(/[xX]/g) || []).length;
      cb = f; for (let i = 0; i < xc - 1; i++) cb += randomInt(0, 9).toString(); tl = bc.length;
    } else { cb = bc; tl = n.lengths[randomInt(0, n.lengths.length - 1)]; }
  } else { cb = n.bins[randomInt(0, n.bins.length - 1)]; tl = n.lengths[randomInt(0, n.lengths.length - 1)]; }
  let cn = cb;
  while (cn.length < tl - 1) cn += randomInt(0, 9).toString();
  if (cn.length > tl - 1) cn = cn.slice(0, tl - 1);
  cn += ((10 - luhnChecksum(cn + '0')) % 10).toString();
  return cn;
}

function generateExpiry(m, y) {
  const now = new Date(), cY = now.getFullYear(), cM = now.getMonth() + 1;
  let eM, eY;
  if (m !== null && y !== null) { if (y < cY || (y === cY && m < cM)) { eY = randomInt(cY, cY+8); eM = eY===cY?randomInt(cM,12):randomInt(1,12); } else { eM=m; eY=y; } }
  else if (m !== null) { eY = randomInt(cY, cY+8); eM = (eY===cY&&m<cM)?randomInt(cM,12):m; }
  else if (y !== null) { eY = y; eM = y===cY?randomInt(cM,12):randomInt(1,12); }
...
function parseExp(exp) {
  let eM = null, eY = null;
  if (exp) {
    const s = String(exp).trim(); let m, y;
    if (s.includes('/')) { [m,y] = s.split('/'); }
    else if (s.length === 4) { m = s.slice(0,2); y = s.slice(2,4); }
    else if (s.length === 6) { m = s.slice(0,2); y = s.slice(2,6); }
    if (m && y) { eM = parseInt(m); eY = String(y).length===2?2000+parseInt(y):parseInt(y); if(eM<1||eM>12)eM=null; if(eY&&(eY<2000||eY>2100))eY=null; }
  }
  return { expMonth: eM, expYear: eY };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < qty; i++) {
      ...
      cards.push({ number: cn, network: CARD_NETWORKS[ch]?.name || 'Unknown', exp_month: m, exp_year: y, expiry: `${m}/${y.slice(2)}`, cvv: cv, bin: cn.slice(0, 6) });
    }
